<?php
	...
	// **** Begin modify config one user one folder  
	 session_start();
		if(isset($_SESSION['folder_user_upload_image'])){
			$folderUser = $_SESSION['folder_user_upload_image'];
		}else
			$folderUser = 'media/images/tinymce/1default_folder';
			
	/*
	if (isset($_COOKIE["USER_IMAGE_PATH"]))
  		$folderUser = $_COOKIE["USER_IMAGE_PATH"];
	else
		$folderUser = 'media/images/tinymce/1default_folder';
	*/
	//$URL_CHECK = "http://".$_SERVER['SERVER_NAME'].$_SESSION['BASE_URL']."/media/images/tinymce";	
	
	// **** End modify config one user one folder
	...
	// trong project này ta lui từ imagemanager tới public::: \public\js\tinyMCE\jscripts\tiny_mce\plugins\imagemanager 
	$mcImageManagerConfig['filesystem.rootpath'] = '../../../../../../'.$folderUser; // absolute or relative from this script path, required.
