...
class Zend_View_Helper_SidebarAdmin {
	...
	function sidebarAdmin($name, $selected) {
	    ...
	    if($role==2||$role==3){
			...
			$flag = false;
			$html .= $this->_element('Bài thi', '/test', 'document--pencil', $flag);
	    }
	    if($role==2||$role==3){
		    // Question Bank
			if ($selected == 'question')
			...
			$flag = false;
			$html .= $this->_element('Ngân hàng câu hỏi', '/question', 'question', $flag);
	    }
	    if($role==2||$role==3){	    
			// Mon hoc
			if ($selected == 'subject')
			...
			$flag = false;
			$html .= $this->_element('Môn học', '/subject', 'ui-scroll-pane-tree', $flag);
	    }
*/
		$html .=  '</ul></div>';
		return $html;
